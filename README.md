...
Decuria 是 [Hermes Agent](https://github.com/NousResearch/hermes-agent) 的独立 Dashboard 插件。它把**多模型圆桌协作、通道模型路由、Provider 管理、模型可见性控制**收进一个面板，不改 Hermes 一行核心代码，装上即用。
...
### 1. 智囊团 · 让模型互相补位，而不是只听一个答案

![智囊团圆桌](docs/screenshots/moa-roundtable.png)

智囊团基于 **Hermes Agent v0.18.2 原生的 Mixture-of-Agents（MoA）能力**打造，把原本只能靠配置文件驱动的多模型协作，升级成一张**拖拽即可编排、全程可视化的专家圆桌**。

把模型拖进「专家席」和「指挥席」，就组成一张圆桌：**最多 8 位专家并行、各自独立作答，再由 1 位指挥模型综合共识、分歧与建议**，最后交付一份结论。
...
在原生 MoA 之上，Decuria 补齐了一整套可视化与编排体验：

- **拖拽式圆桌编排**：谁上场、谁指挥、几位专家，拖一拖就定，无需手写配置文件。
- **专家席 / 指挥席一目了然**：圆桌布局直观呈现每个角色，一眼看懂这场会商是怎么组织的。
- **单轮 / 多轮辩证**：单轮各答一次、指挥汇总，省时省钱；多轮（2–5 轮）让专家看到上一轮观点继续质疑修正，还能开启「高共识提前结束」，收敛即停、不浪费调用。
- **用量总览**：每位专家与指挥各消耗了多少 Token 一目了然，成本心里有数。
- **通讯端一键召集**：在已授权的消息渠道用触发词直接开团，与 Dashboard 共享同一套圆桌与辩证设置。

### 2. 通道配置 · 每个入口，都用最合适的模型

![通道配置](docs/screenshots/channel-config.png)

全局默认模型、各消息渠道、图片 / 视觉 / 视频候补模型，全在一页里配置。模型、Provider、Key 三级联动选择，批量保存，从此告别手改配置文件、逐条重启。

### 3. 模型管理 · 只让真正需要的模型进入视野
